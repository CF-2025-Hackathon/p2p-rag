...
func notifyExternalApiAboutGossipedTopic(expertiseData Expertise, peerId string) {
	...
	l.Info("📡 Notifying external API about gossiped topic:", "from peer: ", peerId, " to ", apiExpertiseUrl)
	...
	go func() {
		...
		if err != nil {
			l.Warn("❌ Failed to marshal topic data:", err)
			return
		}

		resp, err := client.Post(apiExpertiseUrl, "application/json", bytes.NewReader(jsonData))
		if err != nil {
			l.Warn("❌ Failed to notify external API:", err)
			return
		}
		defer resp.Body.Close()

		if resp.StatusCode >= 400 {
			l.Warn("❌ External API returned error status:", resp.StatusCode)
		} else {
			l.Info("✅ Successfully notified external API about topic")
		}
	}()
}
...
func handleQueryStream(stream network.Stream) {
	...
	if err := decoder.Decode(&request); err != nil {
		l.Warn("❌ Error decoding query request:", err)
	...
	l.Info("📥 Received query request from peer:", stream.Conn().RemotePeer())
	l.Info("📥 Request details:", string(requestJson))

	// Forward the query to the local search API
	result, err := forwardQueryToLocalAPI(request)
	if err != nil {
		l.Warn("❌ Error forwarding query to local API:", err)
	...
	if err := encoder.Encode(response); err != nil {
		l.Warn("❌ Error encoding query response:", err)
		return
	}

	if err := rw.Writer.Flush(); err != nil {
		l.Warn("❌ Error flushing response:", err)
		return
	}

	l.Info("📤 Sent query response to peer:", stream.Conn().RemotePeer())
}
...
func forwardQueryToLocalAPI(request QueryRequest) (interface{}, error) {
	...
	//l.Info("📤 Forwarding query to local API with payload:", string(jsonData))
...
func sendErrorResponse(rw *bufio.ReadWriter, errorMsg string) {
	...
	encoder := json.NewEncoder(rw.Writer)
	if err := encoder.Encode(response); err != nil {
		l.Warn("❌ Error encoding error response:", err)
		return
	}

	if err := rw.Writer.Flush(); err != nil {
		l.Warn("❌ Error flushing error response:", err)
	}
}
...
func queryRemotePeer(ctx context.Context, host host.Host, peerIdStr string, request QueryRequest) (interface{}, error) {
	...
	requestJson, _ := json.Marshal(request)
	l.Info("📤 Sending query request to peer:", peerID)
	l.Info("📤 Request details:", string(requestJson))
	...
	l.Info("📤 Sent query request to peer:", peerID)
	...
	l.Info("📥 Received query response from peer:", peerID)
...
func Execute() {
	...
	l.Info("Host created. We are: ", host.ID())
	l.Info(host.Addrs())
	...
	l.Info("Announcing ourselves with rendezvous ", config.RendezvousString)
	...
	for {
		...
		l.Info("Searching for other peers...")
		// Wait again...
		time.Sleep(2 * time.Second)
		peerChan, e := routingDiscovery.FindPeers(ctx, config.RendezvousString)
		if e != nil {
			panic(e)
		}
		for peer := range peerChan {
			...
			if host.Network().Connectedness(peer.ID) != network.Connected {
				l.Info("Connecting to: ", peer.ID, peer.Addrs)
				stream, err := host.NewStream(ctx, peer.ID, protocol.ID(config.ProtocolID))

				if err != nil {
					l.Warn("Connection failed: ", err)
...
func gossipTopics(pubsubTopic *pubsub.Topic) {
	...
	for _, embeddingData := range myExpertise {
		...
		if err != nil {
			l.Warn("❌ Error publishing topic:", err)
			continue
		}

		l.Info("📡 Gossiped topic, with ", len(embeddingData.Embeddings), " vectors")
	}
}
...
func listenForGossip(sub *pubsub.Subscription) {
	for {
		...
		if err != nil {
			l.Warn("❌ Error receiving gossip:", err)
			continue
		}
		l.Info("📩 Received gossip from: ", msg.ReceivedFrom)
		...
		if err := json.Unmarshal(msg.Data, &expertisePayload); err != nil {
			l.Warn("❌ Failed to unmarshal received topic data:", err)
			continue
		}
